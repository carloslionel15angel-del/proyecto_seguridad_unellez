...
# 👤 MODELO GUARDIA (Versión Completa)
class Guardia(models.Model):
    # Relación obligatoria con el usuario de sistema para autenticación
...
class Incidencia(models.Model):
    ...
    archivo_evidencia = models.FileField(upload_to='evidencias_incidencias/', null=True, blank=True, verbose_name="Archivo de Evidencia")

    class Meta:
        ordering = ['-fecha_hora']

# 📅 MODELO SOLICITUDES OPERATIVAS
...
# 💬 MODELO MENSAJE (ACTUALIZADO: Comunicación directa con campo leido)
class Mensaje(models.Model):
    emisor = models.ForeignKey(Guardia, on_delete=models.CASCADE, related_name='mensajes_enviados')
    # Campo para saber a quién va dirigido (Receptor)
    ...
    def __str__(self):
        return f"De {self.emisor.nombre} a {self.receptor.username}: {self.contenido[:20]}"